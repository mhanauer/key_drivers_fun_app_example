...
setwd("/data/data_for_analysis/matthew.hanauer/shiny_360_app_example/src")
source(knitr::purl(glue("/data/data_for_analysis/matthew.hanauer/shiny_360_app_example/src/data_generation_a360.Rmd")))
...
kd <- ggplot(key_drivers_data,aes(importance, positive, label = variable))+
  ...
  labs(x = "Importance", y =  "Favorable") +
  ggtitle(title) +
  annotate(geom="text", x = x_max, y= y_max - y_max*y_adjustment, label="Strength",
           color="#00B0F0") +
  annotate(geom="text", x = x_min , y = y_max-y_max*y_adjustment, label="Unimportant",
           color="#808080") +
  annotate(geom="text",  x=  x_max, y = y_min+y_min*y_adjustment, label="Improvement",
           color="#FF0000") +
  annotate(geom="text", x =  x_min, y= y_min+y_min*y_adjustment, label= "Unimportant",
           color="#808080") + 
