...
```
Then you can use the viz function to product the key drivers graph.  In the top left corner are the improvement opportunities.  Those are drivers above the median in importance, but below to median in favorability.  In the top right are the key strengths which are drivers above the median in both importance and favorability.  In the lower right corner are the drivers below the median importance and favorability.  Finally in the lower right corner are the drivers below the median on importance, but above the median on favorability.
```{r}
